...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"**╰━❖  duchess ping ❖━╯**\n"
                    f"☞ __ping :__ "
                    f"`%sms` \n"
                    f"☞ __i'm online :__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("`.....⚡Duchess⚡.....`")
    await pong.edit("`⚡️`")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"•⎚• ⎆ __Duchess__ **Pong!**\n"
                    f"☞  __Ping :__ "
                    f"`%sms` \n"
                    f"☞  __Sisa Waktu :__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"- D U C H E S S -\n"
                    f"**☞ sinyal  :** "
                    f"`%sms` \n"
                    f"**☞ i'm online  :** "
                    f"`{uptime}` \n"
                    f"__|━|⎆__ **My Duchess  :** `{ALIVE_NAME}`" % (duration))
...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("__D U C H E S S__")
    await pong.edit("__PONG__")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"┏━━| **DUCHESS PONG** |━━卍\n"
                    f"┣|•  __Ping :__ "
                    f"`%sms` \n"
                    f"┗|• __Uptime :__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"**Duchess** - {ALIVE_NAME}\n\n"
                    f"**Pong !!** "
                    f"`%sms` \n"
                    f"**Uptime** - "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    await pong.edit("⎚⎆ __Duchess__ **Test Ping!**\n`%sms`" % (duration))
